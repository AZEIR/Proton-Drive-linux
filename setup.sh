...
# ── 1. Build if binary is missing ────────────────────────────────────────────
if [ ! -f "$BINARY" ]; then
    echo "Binary not found — building now..."
    echo ""

    # Locate bun: prefer system install, then local node_modules
    BUN_BIN=""
    ...
    if [ -z "$BUN_BIN" ]; then
        echo "ERROR: bun is required to build Proton Drive but was not found."
    ...
    (cd "${SCRIPT_DIR}/sdk/js/cli" && "$BUN_BIN" install --frozen-lockfile 2>&1)
    (cd "${SCRIPT_DIR}/sdk/js/cli" && "$BUN_BIN" run build 2>&1)

    if [ ! -f "$BINARY" ]; then
        echo ""
        echo "ERROR: Build finished but binary still missing at:"
        echo "  ${BINARY}"
        exit 1
    fi
    echo "Build complete."
...
# Substitute the real home path into ExecStart (the service file uses %h which
# works for systemd but we store an explicit path for robustness).
...
if grep -q "^Environment=PROTON_MOUNT_POINT=" "$SERVICE_DST" 2>/dev/null; then
    ...
    # Add after the existing Environment line (or at end of [Service] block)
